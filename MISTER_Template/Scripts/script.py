...
def write_Game(text):
    #split up the file ['Azurian Attack', '/media/rmaFolder/Azurian%20Attack.mra', '/media/rbfFolder/jtcps1_20220819.rbf','https://archive.org/download/MAME220RomsOnlyMerged/azurian.zip', 'games/mame/azurian.zip\n']
    game = text.split(',')
    ...
    if os.path.exists('../_Arcade/cores/'+rbf):
        ...
        r5 = requests.get('http://127.0.0.1:8000/media/rbfFolder/' + rbf)
    ...
    for i in range(num_files):
        file_link.append(game[i+3]) #plus 3 start at the first download link skip 'game_name' and 'rma' and rbf
    
    for i in range(num_files):
        file_path.append(game[(num_files+i+3)]) #plus 2 start at the first download link skip 'game_name' and 'rma' and rbf
...
def delete_Game(text):
    ...
    for i in range(num_files):
        file_path.append(game[(num_files+i+3)]) #plus 3 start at the first download link skip 'game_name' and 'rma' and rbf
                                          #plus num_files to skip over the file_link
...
payload = {'value':'0bc860fd-5d17-4b7a-aa6f-d6315975aab0'}
...
#-------output the web server list to a file and add it to the hash table h_web. 
        ...
        for x in range(web_num_games):
            output = (server_list[x]['game_name']+","+server_list[x]['rma_file']+","+server_list[x]['rbf_file']+","+server_list[x]['game_file_link']+","+server_list[x]['file_location']+"\n")
            f.write(output)
